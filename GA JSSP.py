...
def run():
    re_entry = "JSP_dataset_re_entry_SP_50J.xlsx"
    ...
    parameters_input_tmp = pd.read_excel(using, sheet_name="parameters", index_col=[0])

    dfshape = pt_tmp.shape
    num_tasks = dfshape[0]
    num_jobs = ms_tmp.shape[1]
    num_gated_machines = m_dep_tmp.shape[1]
    num_runs = parameters_input_tmp.shape[0]
    # number of jobs
    ...
    total_runs = [list(parameters_input_tmp.iloc[i]) for i in range(num_runs)]
    machines = [list(map(int, machine_parallel.iloc[i])) for i in range(1)][0]
    num_gene = num_tasks * num_jobs  # number of genes in a chromosome
    ...
    for job in machine_dependency:
        for j in range(num_gated_machines):
            stringfy = str(job[j])
            gates = list(stringfy)
    ...
    results = []  #
    for idx, run in enumerate(total_runs):
        # raw_input is used in python 2
        population_size = int(run[0])  # default value is 30
        crossover_rate = float(run[1])  # default value is 0.8
        mutation_rate = float(run[2])  # default value is 0.2
        mutation_selection_rate = float(run[3])
        num_mutation_jobs = round(num_gene * mutation_selection_rate)
        num_iteration = int(run[4])  # default value is 2000

        shifting = str(run[5])
        shifting_rate = float(run[6])
        shifting_blocks = int(run[7])

        jhonson = str(run[8])
        jhonson_activation_rate = float(run[9])
        jhonson_mutation_rate = float(run[10])
        ...
        for n in range(num_iteration):
                    ...
                    while job_count[chg_job][0] > num_jobs:
                        for d in range(len(less)):
                            if job_count[less[d]][0] < num_jobs:
                                ...
                                offspring_decimals[m][job_count[chg_job][1]] = np.random.uniform(low=0, high=1,
                                                                                                 size=(1,))
                    ...
                    if probability < shifting_rate:
                        for i in range(num_gene):
                            if offspring_list[m][i] == idx_max_process:
                                ...
                                new_idx = max(0, i - random.randint(0, shifting_blocks))
            ...
            '''--------AntiPotato shifting-------------'''
            '''
            For each offspring, we pick for each machine two random jobs. We put the job with less runtime first.
            '''
            if jhonson != 'False':
                for m in range(population_size):
                    probability = np.random.rand()
                    # probability that a gen is mutated.
                    if probability < jhonson_activation_rate:

                        for i in range(len(machines)):
                            # probability that jobs are swapped per machine.
                            mutation_prob = np.random.rand()
                            if mutation_prob < jhonson_mutation_rate:
                                ...
                                if processing_time[candidates[0]][i] > processing_time[candidates[1]][i] and \
                                        job_index[candidates[0]] < job_index[candidates[1]]:
                                    tmp_c = offspring_list[m][job_index[candidates[1]]]
                                    offspring_list[m][job_index[candidates[1]]] = offspring_list[m][job_index[candidates[0]]]
                                    offspring_list[m][job_index[candidates[0]]] = tmp_c
        ...
        plt.show()
        timebest = (time.time() - start_time)

        results.append([Tbest, timebest])
